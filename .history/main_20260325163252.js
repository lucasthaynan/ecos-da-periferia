...
async function carregarCatalogo() {
    try {
        // Assume que 'dados.json' está na pasta 'data' relativa ao HTML
...
function renderizarCatalogo(dados) {
    ...
    // 3. Ordena os segmentos pela quantidade de artistas (maior para menor)
    const segmentosOrdenados = Object.entries(segmentos).sort((a, b) => {
        return b[1].length - a[1].length;
    });

    // 4. Cria o HTML para cada segmento
    segmentosOrdenados.forEach(([segmento, artistas]) => {
        ...
        artistas.forEach(artista => {
            ...
            divArtista.dataset.sobre = sobreHtml;
...
function abrirPopup(data) {
    ...
    popup.querySelector('.imagem p.credito').innerHTML = `<strong>Crédito:</strong> ${data.credito || 'Não informado'}`;
